...
import { noAirflowGateQuestions, routeLabels } from '@/features/diagnostic/config';
...
export default function NoAirflowGatesScreen() {
  ...
  const isComplete = noAirflowGateQuestions.every((question) => {
  ...
  const handleContinue = () => {
    ...
    if (resolution.shouldAskFocusedDiagnostic && resolution.route === 'indoor_unit_diag') {
  ...
  return (
    <AppScreen
      ...
      <FlowHeader
        title="No Airflow gate checks"
        subtitle="Only ask what is needed to place the first subsystem route."
        currentStep={5}
      ...
      <ProgressiveQuestionFlow
        questions={noAirflowGateQuestions.map((question) => ({
          ...question,
          key: question.key,
