...
import { routeLabels, systemIdleGateQuestions } from '@/features/diagnostic/config';
...
export default function SystemIdleGatesScreen() {
  ...
  useEffect(() => {
    if (session.issue !== 'system_not_doing_anything') {
  ...
  const isComplete = systemIdleGateQuestions.every((question) => {
  ...
  return (
    <AppScreen
      ...
      <FlowHeader
        title="System idle gate checks"
        subtitle="Start with the cleanest control and power separators."
        currentStep={5}
      ...
      <ProgressiveQuestionFlow
        questions={systemIdleGateQuestions.map((question) => ({
          ...question,
          key: question.key,
