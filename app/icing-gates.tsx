...
import { icingGateQuestions, routeLabels } from '@/features/diagnostic/config';
...
export default function IcingGatesScreen() {
  ...
  useEffect(() => {
    if (session.issue !== 'icing_frozen_coil') {
  ...
  const isComplete = icingGateQuestions.every((question) => {
  ...
  return (
    <AppScreen
      ...
      <FlowHeader
        title="Icing gate checks"
        subtitle="Use only the fast freeze-up separators before choosing the next branch."
        currentStep={5}
      ...
      <ProgressiveQuestionFlow
        questions={icingGateQuestions.map((question) => ({
          ...question,
          key: question.key,
