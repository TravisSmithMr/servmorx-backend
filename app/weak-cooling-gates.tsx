...
import { routeLabels, weakCoolingGateQuestions } from '@/features/diagnostic/config';
...
export default function WeakCoolingGatesScreen() {
  ...
  const isComplete = weakCoolingGateQuestions.every((question) => {
  ...
  return (
    <AppScreen
      ...
      <FlowHeader
        title="Weak Cooling gate checks"
        subtitle="Use the fastest gates that separate airflow from refrigeration."
        currentStep={5}
      ...
      <ProgressiveQuestionFlow
        questions={weakCoolingGateQuestions.map((question) => ({
          ...question,
          key: question.key,
