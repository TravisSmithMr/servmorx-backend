...
import { outdoorUnitNotRunningGateQuestions, routeLabels } from '@/features/diagnostic/config';
...
export default function OutdoorUnitNotRunningGatesScreen() {
  ...
  useEffect(() => {
    if (session.issue !== 'outdoor_unit_not_running') {
  ...
  const isComplete = outdoorUnitNotRunningGateQuestions.every((question) => {
  ...
  const handleContinue = () => {
    ...
    if (resolution.shouldAskFocusedDiagnostic && resolution.route === 'outdoor_unit_diag') {
  ...
  return (
    <AppScreen
      ...
      <FlowHeader
        title="Outdoor unit gate checks"
        subtitle="Confirm the condenser lane before going deeper into the outdoor section."
        currentStep={5}
      ...
      <ProgressiveQuestionFlow
        questions={outdoorUnitNotRunningGateQuestions.map((question) => ({
          ...question,
          key: question.key,
