...
function buildFallbackContext(context: DiagnosticsContextPayload) {
  return {
    ...
    issue: context.issue,
    route: context.route,
    primaryRoute: context.primaryRoute ?? null,
    secondaryRoute: context.secondaryRoute ?? null,
...
export async function handleDiagnosticsCopilot(
  ...
  try {
    ...
    const reason = error instanceof Error ? error.message : 'Unknown backend copilot error.';

    return {
      provider: 'backend_local_fallback',
      providerPath: 'backend_proxy',
      providerStatus: `Backend copilot fallback used: ${reason}`,
      usedFallback: true,
